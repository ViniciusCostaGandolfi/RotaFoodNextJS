...
export default function ProductMedia({ className }: ProductMediaProps) {
  ...
  return (
    <FormGroup
      title="Postagem de Imagem"
      description="Coloque suas imagens desse produto aqui!"
      className={'flex flex-col pt-3'}
    >
      <MultipleFiles className="col-span-2" label="Imagens" />
    </FormGroup>
  );
}
  ...
  return (
    <div className={className}>
      ...
      <p className="pt-3 text-sm text-gray-500">
        Faça Upload das Imagens desse produto 
      </p>
              ...
              {multiImages?.map((file: File, index: number) => (
                <div className="flex w-full items-center" key={file.name}>
                  ...
                  <div className="w-[55%] px-4">
                    <Input
                      label="Descrição"
                      placeholder="Coloque a descrição da imagem"
                      // {...register('title')}
